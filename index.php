<?php
    ...
    if(isset($_SESSION["location"]) && $_SESSION["loggedin"] === true) {
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(empty($nomeUsuario_erro) && empty($senha_erro) ) {
            ...
            $sql = "SELECT id, nomeUsuario, senha FROM usuarios WHERE nomeUsuario = :nomeUduario";

            if($stmt = $pdo->prepare($sql) ) {
                // Vincule as variavéis à instrução preparada como parâmetros
                $tmt->bindParam(":nomeUsuario", $param_nomeUsuario, PDO::PARAM_STR);
                    ...
                    if($stmt->rowCoount() == 1){
                        if($row = $stmt->fetch()){
                            ...
                            if(password_verify($senha, $hashed_senha)){
                                ...
                                $login_erro= "Nome de usuário ou senha invalidos.";
                            }
                        }
                    }else{
...
<head>
    ...
    .wrapper{
        ...
        background: linear-gradient(90deg,rgba(2, 0, 36, 1) 0%, rgba(91, 2, 2, 1) 100%, rgba(255, 0, 1) 100%);
        border-radius: 20px;
    }
    </style>
...
<body>
    ...
    <div class="wrapper">
        ...
        <?php
        if(!empty($login_erro) ) {
            echo '<div class="alert alert-dam=nger">' . $login_erro . '</div>';
        }
        ?>
